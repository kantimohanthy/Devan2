...
export function ContextInspector() {
  ...
  const executionStacks = context.executionStacks || [];
  const evidenceList = context.evidenceList || [];
  const diagnosticTools = context.diagnosticTools || [];
  const rfcsAndManPages = context.rfcsAndManPages || [];
  const careerImpact = context.careerImpact || [];
      ...
      {executionStacks.length > 0 && (
        <div className="space-y-2 border-b border-[#20252B] pb-3">
          ...
          <div className="rounded bg-[#121418] p-2 border border-[#20252B] space-y-1">
            <div className="text-[11px] text-[#F5F5F5] font-semibold">{executionStacks[0].title}</div>
            <div className="text-[10px] text-[#8A9098]">{executionStacks[0].purpose}</div>
          </div>
        </div>
      )}
